...
---------------------------------------------------------------*/
static fsm_state_base* lag_fsm_state_dispatcher( lag_fsm  * fsm, uint16 state );
/*----------------------------------------------------------------
...
---------------------------------------------------------------*/
 ...
 fsm_state_base      state_lag_fsm_idle ={ "idle", lag_fsm_idle , SIMPLE, NULL, NULL, &lag_fsm_state_dispatcher, NULL, NULL };

 fsm_state_base      state_lag_fsm_admin_down ={ "admin_down", lag_fsm_admin_down , SIMPLE, NULL, NULL, &lag_fsm_state_dispatcher, admin_down_entry_func , NULL };

 fsm_state_base      state_lag_fsm_admin_up ={ "admin_up", lag_fsm_admin_up ,  COMPOSED, NULL, &state_lag_fsm_lag_down, &lag_fsm_state_dispatcher, NULL, NULL };

 fsm_state_base      state_lag_fsm_lag_down ={ "lag_down", lag_fsm_lag_down , SIMPLE, &state_lag_fsm_admin_up, NULL, &lag_fsm_state_dispatcher, lag_down_entry_func , lag_down_exit_func };

 fsm_state_base      state_lag_fsm_wait_lag_up ={ "wait_lag_up", lag_fsm_wait_lag_up , SIMPLE, &state_lag_fsm_admin_up, NULL, &lag_fsm_state_dispatcher, NULL, NULL };

 fsm_state_base      state_lag_fsm_lag_up ={ "lag_up", lag_fsm_lag_up , SIMPLE, &state_lag_fsm_admin_up, NULL, &lag_fsm_state_dispatcher, NULL, NULL };

 fsm_state_base      state_lag_fsm_condit1 ={ "condit1", lag_fsm_condit1 ,  CONDITION ,NULL, NULL, &lag_fsm_state_dispatcher, NULL, NULL};

 fsm_state_base      state_lag_fsm_mismatch ={ "mismatch", lag_fsm_mismatch , SIMPLE, &state_lag_fsm_admin_up, NULL, &lag_fsm_state_dispatcher, NULL, NULL };
   ...
   case lag_fsm_admin_up : 
    {
      ...
      else if ( event->opcode == port_up_ev )
      {
         ...
         {/*tr21:*/
            fsm->base.err = on_port_up_composed_state( fsm, DEFAULT_PARAMS_D);  /* Source line = 76*/ 
            return &state_lag_fsm_admin_up;
         }
      }
      ...
      else if ( event->opcode == lacp_down_ev )
      {
         ...
         {/*tr22:*/
            fsm->base.err = on_lacp_down_composed_state( fsm, DEFAULT_PARAMS_D);  /* Source line = 77*/ 
            return &state_lag_fsm_admin_up;
         }
      }
      ...
      else if ( event->opcode == lacp_up_ev )
      {
         ...
         {/*tr23:*/
            fsm->base.err = on_lacp_up_composed_state( fsm, DEFAULT_PARAMS_D);  /* Source line = 78*/ 
            return &state_lag_fsm_admin_up;
         }
      }
   ...
   case lag_fsm_lag_down : 
    {
      if ( event->opcode == port_up_ev )
      {
         ...
         {/*tr31:*/
            fsm->base.err = on_port_up_lacp_disable( fsm, DEFAULT_PARAMS_D);  /* Source line = 90*/ 
      ...
      else if ( event->opcode == lacp_up_ev )
      {
         SET_EVENT(lag_fsm , lacp_up_ev)
         {/*tr32:*/
            fsm->base.err = on_lacp_up_in_wait_state( fsm, DEFAULT_PARAMS_D);  /* Source line = 91*/ 
            return &state_lag_fsm_lag_up;
         }
      }
   ...
   case lag_fsm_wait_lag_up : 
    {
      ...
      else if ( event->opcode == lacp_up_ev )
      {
         SET_EVENT(lag_fsm , lacp_up_ev)
         {/*tr41:*/
            fsm->base.err = on_lacp_up_in_wait_state( fsm, DEFAULT_PARAMS_D);  /* Source line = 103*/ 
            return &state_lag_fsm_lag_up;
         }
      }
...
int lag_fsm_init(struct lag_fsm *fsm, fsm_user_trace user_trace, void * sched_func, void * unsched_func)
 {
       ...
       fsm_init(&fsm->base,  default_state, 8, fsm->state_timer, user_trace, sched_func, unsched_func);
...
tbool lag_fsm_admin_up_in(struct lag_fsm *fsm){
    return  fsm->base.curr_state->composed && (fsm->base.curr_state->composed->id == lag_fsm_admin_up)  ;
}
...
// Returns:  none
//-----------------------------------------------------------------------
int    on_admin_on (lag_fsm  * theFsmP, int parameter )
 {
...
// Returns:  none
//-----------------------------------------------------------------------
int   on_admin_off ( lag_fsm  * theFsmP,int parameter )
 {
  // lag->HandleAdminStatusChange();
...
int  on_port_up_composed_state ( lag_fsm *fsm, int parameter )
 {
   SET_EVENT(lag_fsm , port_up_ev) ;
    return 0;
 }
...
// Returns:  none
//-----------------------------------------------------------------------
int   on_port_up_lacp_enable ( lag_fsm  * fsm, int parameter )
{
   return 0;
}
...
// Returns:  none
//-----------------------------------------------------------------------
int  on_port_up_lacp_disable ( lag_fsm  * fsm, int parameter )
{
   return 0; 
}
...
int   on_port_down_last_port ( lag_fsm  * theFsmP, int parameter )
{
   return 0;
}
...
int   on_in_progress_tout (lag_fsm  * theFsmP,  int parameter )
{
   return 0;
}
...
// Returns:  none
//-----------------------------------------------------------------------
int  on_mismatch (lag_fsm  * fsm,  int parameter )
{
   SET_EVENT(lag_fsm , mismatch_ev) ;
    return 0;
}
...
// Returns:  none
//-----------------------------------------------------------------------
int  on_lacp_up_in_wait_state (lag_fsm  * fsm, int parameter )
{
  SET_EVENT(lag_fsm , lacp_up_ev) ;
   return 0;   
}
 ...
 //----------------------------------------------------------------------
  ...
  // Returns:  none
  //-----------------------------------------------------------------------
int  on_lacp_down_last_port (lag_fsm  * fsm, int parameter )
{
  SET_EVENT(lag_fsm , lacp_down_ev) ;
   return 0;
}
...
// Returns:  none
//-----------------------------------------------------------------------
int   on_lacp_down_composed_state (lag_fsm  * theFsmP, int parameter )
{
	 return 0;
}
...
// Returns:  none
//-----------------------------------------------------------------------
int   on_lacp_up_composed_state (lag_fsm  * theFsmP, int parameter )
{
	 return 0;
}


static int admin_down_entry_func(lag_fsm *pFsm )
{
    printf("called %s\n",__FUNCTION__);
	return 0;
}
static int lag_down_entry_func(lag_fsm *pFsm )
{
	printf("called %s\n",__FUNCTION__);
	return 0;
}
static int lag_down_exit_func(lag_fsm *pFsm )
{
	printf("called %s\n",__FUNCTION__);
	return 0;
}
